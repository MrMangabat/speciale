...
## errors
from urllib.request import urlopen, sys, time
...
class DrIndland(scrapy.Spider):
    ...
    def parse(self, response):
        rss_feed = response.xpath('//item/guid').get()
    ...
    def parse_article_body(self, response, article_title, published, id, link):
        ...
        for text_body in article:
            # print('------------------------INSIDE FOR LOOP DRINDLAND------------------------------')
        ...
        indland_regex = re.compile(r'https://www.dr.dk/nyheder/indland/.*')
        udland_regex = re.compile(r'https://www.dr.dk/nyheder/udland/.*')
        politik_regex = re.compile(r'https://www.dr.dk/nyheder/politik/.*')

        if penge_regex.match(link):
            article_item['tag'] = 'penge'

        elif indland_regex.match(link):
            article_item['tag'] = 'indland'

        elif udland_regex.match(link):
            article_item['tag'] = 'udland'

        elif politik_regex.match(link):
